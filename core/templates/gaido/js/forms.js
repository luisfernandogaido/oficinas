...
const avaliaForm = f => {
  ...
  f.querySelectorAll('input, select, textarea, form').forEach(el => {
    ...
    if (el.type == 'radio') {
      el.addEventListener('click', updateValidListener);
    }
    f.observer.disconnect();
    if (el.required) {
      el.parentNode.parentNode.classList.add('required');
      el.closest('.campo')?.classList.add('required');
    } else {
      el.closest('.campo')?.classList.remove('required');
    }
    f.observer.observe(f, {attributes: true, childList: true, subtree: true});
  });
  f.querySelectorAll('input.money').forEach(el => {
    el.inputmode = 'numeric';
    el.addEventListener('keydown', ev => {
      const key = ev.key;
      const keys = '0123456789';
      if (key.length != 1) return;
      if (!keys.includes(key)) ev.preventDefault();
    });
    el.addEventListener('input', ev => {
      ...
      const digits = parseInt(el.dataset.decimalPlaces ?? '2');
      const value = el.value.
          replaceAll('.', '').
          replaceAll(',', '');
      const num = parseFloat(value) / 10 ** digits;
      if (isNaN(num)) {
        return 'Formato de moeda inválido.';
      }
      el.value = num.toLocaleString('pt-BR', {minimumFractionDigits: digits});
...
const setMensagem = (el, msg = '') => {
  const ctnMsg = el.closest('.campo').querySelector('.mensagem');
  if (!ctnMsg) return;
...
const post = async (f, url) => {
  ...
  if (r.erro) throw new Error(r.mensagem ?? r.erro);
  return r;
};
