...
const validacoesPadrao = [
  ...
  },
  {
    ...
    val: el => {
      if (!el.value) return;
      let digitos = '0123456789';
      let s = [...el.value].filter(c => digitos.includes(c)).join('');
      if (s.length > 11 && s.startsWith('55')) {
        s = s.substring(2);
      }
      if (s.length == 10) {
  ...
  },
  {
    ...
    val: el => {
      if (!el.value) return;
      if (!el.step) return;
      const i = el.step.indexOf('.');
      if (i == -1) return;
      const casas = el.step.length - i - 1;
      el.value = parseFloat(el.value).toFixed(casas);
    },
  },
  {
    sel: '.placa',
    val: el => {
      ...
      const s = el.value.replace(regex, '').toUpperCase();
      el.value = s;
      if (!el.value.match(/^[a-zA-Z]{3}-?[0-9][a-zA-Z0-9][0-9]{2}$/)) {
        return 'Formato de placa precisa ser : AAA0000 ou AAA0A00.';
      }
    },
  },
];
...
const cpf = s => {
  if (!s) return false;
  if (!s.match(/^[0-9]{3}\.?[0-9]{3}\.?[0-9]{3}-?[0-9]{2}$/)) {
    return false;
  }
