...
const get = async () => {
  try {
    ...
    const navigatorPropertites = [];
    for (let k in navigator) {
      navigatorPropertites.push(k);
    }
    ...
    if (canvasContext) {
      const debugInfo = canvasContext.getExtension('WEBGL_debug_renderer_info');
      if (debugInfo) {
        webglRenderer = canvasContext.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL);
        webglExtensions = canvasContext.getSupportedExtensions();
      }
    }
    ...
    audioFormats.forEach(format => {
      audios.push({
        format,
        canPlayType: audio.canPlayType(format),
      });
    });
    let mediaDevices = [];
    ...
    const data = {
      ...
      content_language: window.navigator.languages,
      fonts: [...fontAvailable.values()],
      navigator_properties: navigatorPropertites,
      ...
      screen_avail_height: screen.height,
...
export {get, getWebGLFingerprint};
